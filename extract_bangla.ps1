...
$htmlDir = "D:\Tanvir Mahfuz\80-20-exam\docs\web\ssc-bangla"
$outputDir = "D:\Tanvir Mahfuz\80-20-exam\public\ssc\bangla"

$bnToEn = @{ "ক" = "A"; "খ" = "B"; "গ" = "C"; "ঘ" = "D" }
$enLetters = @("A", "B", "C", "D")

$htmlFiles = Get-ChildItem -LiteralPath $htmlDir -Filter "*.html" | Sort-Object { [int][regex]::Match($_.BaseName, '\d+').Value }
    ...
    while ($true) {
        ...
        if ($hasSub) {
            ...
            if ($subSectionStart -ge 0) {
                ...
                if ($subSectionStart -ge 0) {
                    # Find end of sub-question section
                    $subSectionEnd = $blockHtml.IndexOf('</div></div>', $subSectionStart)
                    if ($subSectionEnd -ge 0) {
                        # Try to find matching end (there might be nested divs)
                        for ($i = 0; $i -lt 10; $i++) {
...
function Get-Options {
    ...
    # Find the content after the grid div tag
    ...
    while ($true) {
        ...
        # Extract text from <p> inside <div class="flex-1"
        $btnText = ""
    ...
    # Fallback: try to find answer as text in explanation
...
foreach ($htmlFile in $htmlFiles) {
    Write-Host "$($htmlFile.Name) ... " -NoNewline
    ...
    try {
        ...
        Write-Host "$($questions.Count) q" -ForegroundColor Green
    }
    catch {
...
Write-Host "`n=== DONE: $total questions ===" -ForegroundColor Cyan
