...
$htmlDir = "D:\Tanvir Mahfuz\80-20-exam\docs\web\hsc-management-2nd-paper"
$outputDir = "D:\Tanvir Mahfuz\80-20-exam\public\hsc\management_2nd"

$bnToEn = @{ "ক" = "A"; "খ" = "B"; "গ" = "C"; "ঘ" = "D" }
$enLetters = @("A", "B", "C", "D")

$htmlFiles = Get-ChildItem -LiteralPath $htmlDir -Filter "*.html"
...
function Get-Options {
    param([string]$blockHtml, [string]$questionText, [int]$qid, [string]$boardName)
    ...
    while ($true) {
        ...
        $texts += $btnText
        if ($btnSlice -match 'bg-\[#017A471A\]' -or $btnSlice -match 'bg-\[#F59E0B1F\]') {
            $correctIdx = $btnIndex
        }
        $btnIndex++
        $btnPos = $btnEnd + 9
...
function Get-QuestionsFromHtml {
    ...
    while ($true) {
        ...
        if ($hasSub) {
                ...
                if ($subSectionStart -ge 0) {
                    ...
                    if ($subSectionEnd -ge 0) {
                        for ($i = 0; $i -lt 10; $i++) {
                            ...
                            if ($nextEnd -ge 0 -and ($nextEnd - $subSectionEnd) -lt 30) {
                                $subSectionEnd = $nextEnd
                            } else { break }
                        }
                            ...
                            if ($sqStart -ge 0) {
                                ...
                                if ($sqEnd -gt $sqStart) {
                                    ...
                                    if (![string]::IsNullOrWhiteSpace($subQText)) {
                                        ...
                                        $qObj = Get-Options -blockHtml $subHtml -questionText $fullText -qid $nextId -boardName $boardName
            ...
            $qObj = Get-Options -blockHtml $blockHtml -questionText $mainQuestion -qid $nextId -boardName $boardName
            if ($qObj) { $questions += $qObj; $nextId++ }
        }
...
foreach ($htmlFile in $htmlFiles) {
    $processed++
    try {
        $content = [System.IO.File]::ReadAllText($htmlFile.FullName, $encoding)
        $boardMatch = [regex]::Match($content, '<h1[^>]*>([^<]+)</h1>')
        if (!$boardMatch.Success) {
            Write-Output "[$processed/$totalFiles] $($htmlFile.Name) -> NO HEADER"
            continue
        }
        $boardName = $boardMatch.Groups[1].Value
        $questions = Get-QuestionsFromHtml -html $content -boardName $boardName
        if ($questions.Count -eq 0) {
            Write-Output "[$processed/$totalFiles] $($htmlFile.Name) -> NO QUESTIONS"
            continue
        }
        $outFile = Join-Path $outputDir "$($htmlFile.BaseName).json"
        $json = $questions | ConvertTo-Json -Depth 3
        [System.IO.File]::WriteAllText($outFile, $json, $encoding)
        $total += $questions.Count
        Write-Output "[$processed/$totalFiles] $($htmlFile.Name) -> $($questions.Count) questions"
    }
    catch {
        Write-Output "[$processed/$totalFiles] $($htmlFile.Name) -> ERROR: $_"
        $errors += "$($htmlFile.Name): $_"
    }
}

Write-Output "`n=== DONE: $total questions from $processed files ==="
