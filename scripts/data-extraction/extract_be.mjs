...
const srcDir = 'D:\\Tanvir Mahfuz\\80-20-exam\\docs\\web\\ssc-business-entrepre';
const outDir = 'D:\\Tanvir Mahfuz\\80-20-exam\\public\\ssc\\business_entrepreneurship';
...
function extractQuestions(html, source) {
  ...
  for (const block of blocks) {
    ...
    if (greenMatch) {
      ...
      if (letterMatch) correctAnswer = bnMap[letterMatch[1]] || '';
    }
    ...
    if (!correctAnswer) {
      ...
      if (ansMatch) correctAnswer = bnMap[ansMatch[1]] || '';
    }

    if (!correctAnswer) {
      const ansRegex2 = /সঠিক উত্তর[ঃ:\s]*(?:হলো\s*)?([ক-ঘ])/;
      const ansMatch2 = block.match(ansRegex2);
      if (ansMatch2) correctAnswer = bnMap[ansMatch2[1]] || '';
    }
...
const mappingPath = join(outDir, '_mapping_be.txt');
