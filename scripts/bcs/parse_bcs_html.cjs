...
if (!bcsNumber || bcsNumber < 10 || bcsNumber > 50) {
  ...
  console.error('  Reads HTML from stdin or from saved file');
  process.exit(1);
}

const OUTPUT_PATH = path.join(__dirname, '..', 'public', 'bcs', `bcs_${bcsNumber}.json`);
const INPUT_PATH = path.join(__dirname, '..', 'temp', `bcs_${bcsNumber}.html`);
...
// Read from stdin or from saved file
let inputData = '';
if (process.argv.includes('--file')) {
  inputData = fs.readFileSync(INPUT_PATH, 'utf-8');
} else {
  // Read stdin
  inputData = fs.readFileSync(process.stdin.fd, 'utf-8');
}
