    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-white/40 text-sm font-medium">Loading questions...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <button onClick={goBack} className="px-6 py-2 bg-white/5 hover:bg-white/10 rounded-xl text-sm font-medium text-white/60 transition-colors">
            Go back
          </button>
        </div>
      </div>
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-white/40 text-lg font-medium">No creative questions available</p>
          <button onClick={goBack} className="px-6 py-2 bg-white/5 hover:bg-white/10 rounded-xl text-sm font-medium text-white/60 transition-colors">
            Go back
          </button>
        </div>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={toggleAll}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 hover:bg-white/10 rounded-xl text-xs font-medium text-white/50 transition-colors"
        >
        ...
        {data.questions.map((q, idx) => (
          ...
          >
            ...
            <div className="p-4 md:p-6 space-y-3">
              <div className="flex items-center gap-2 text-xs font-black text-white/20">
                ...
                {q.source && (
                  <>
                    ...
                    <span className="text-white/30">{q.source}</span>
                  </>
                )}
              </div>
              ...
              {/* Stimulus */}
              {q.stimulus && (
                <div className="p-3 md:p-4 bg-white/[0.02] border border-white/[0.05] rounded-xl text-sm text-white/80 leading-relaxed">
                  {q.stimulus}
                </div>
              )}
              ...
              <div className="space-y-2">
                {q.questions.map(sq => (
                  <div key={sq.label} className="flex items-start gap-2 text-sm">
                    ...
                    <span className="text-white/70">{sq.text}</span>
                    {sq.mark && (
                      <span className="text-[10px] font-black text-white/20 shrink-0 mt-0.5">
                        {sq.mark}
                      </span>
                    )}
                  </div>
            ...
            {Object.keys(q.answer).length > 0 && (
              <div className="border-t border-white/[0.06]">
                <button
                  ...
                  className="w-full flex items-center justify-between px-4 md:px-6 py-2.5 text-xs font-black uppercase tracking-[0.15em] text-white/30 hover:text-white/50 transition-colors"
                >
                  <span>Answers</span>
                      ...
                      return (
                        <div key={sq.label} className="p-2.5 bg-primary/[0.03] border border-primary/[0.08] rounded-lg">
                          <div className="flex items-start gap-2 text-sm">
                            ...
                            <span className="text-white/60 text-xs leading-relaxed">{ans}</span>
                          </div>
                        </div>
                      );
                    })}
