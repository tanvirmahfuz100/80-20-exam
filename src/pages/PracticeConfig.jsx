...
const PracticeConfig = () => {
    ...
    if (error) return (
        <div className="p-6 md:p-10 bg-white border border-wolf rounded-2xl md:rounded-3xl flex flex-col items-center gap-4 text-center">
            <div className="p-4 bg-primary/10 rounded-full">
                <Sparkles className="w-8 h-8 md:w-10 md:h-10 text-primary" />
            </div>
            <div>
                <h3 className="text-charcoal font-black text-lg md:text-xl tracking-tighter">পথ বিরতি</h3>
                <p className="text-hare text-sm max-w-sm mx-auto mt-2 font-medium">{error}। আমরা লেসন লোড করতে পারিনি।</p>
            </div>
        </div>
    );
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="max-w-3xl mx-auto space-y-6">
            <motion.div
                ...
                className="relative overflow-hidden rounded-2xl bg-white border border-wolf p-5 md:p-6 flex flex-col md:flex-row md:items-end justify-between gap-3 shadow-sm"
            >
                <div className="min-w-0 flex-1 relative z-10">
                    <motion.h1
                        ...
                        className="text-xl md:text-3xl font-black text-charcoal tracking-tight mb-1"
                    >
                    ...
                    <motion.p
                        ...
                        className="text-hare font-medium text-xs"
                    >
                ...
                <motion.div
                    ...
                    className="bg-eel border border-wolf p-0.5 rounded-xl flex items-center gap-0.5 self-start md:self-end shrink-0 relative z-10"
                >
                    <button onClick={() => setIsTimed(false)} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-[10px] font-bold tracking-wider transition-all whitespace-nowrap ${!isTimed ? 'bg-primary text-white shadow-sm' : 'text-hare hover:text-charcoal'}`}>
                        আনটাইমড
                    </button>
                    <button onClick={() => setIsTimed(true)} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-[10px] font-bold tracking-wider transition-all whitespace-nowrap ${isTimed ? 'bg-primary text-white shadow-sm' : 'text-hare hover:text-charcoal'}`}>
            ...
            <motion.div
                ...
                className="flex items-center gap-1 md:gap-2 text-[10px] font-bold uppercase tracking-[0.12em] py-2.5 md:py-3 px-3 md:px-4 bg-white border border-wolf rounded-xl overflow-x-auto no-scrollbar"
            >
                {steps.map((s, i) => (
                    <React.Fragment key={s.key}>
                        {i > 0 && <ChevronRight className="w-3 h-3 text-hare" />}
                        <motion.button
                            ...
                            className={`flex items-center gap-1.5 transition-all px-2.5 py-1.5 rounded-full text-[10px] font-bold ${i === step
                                ...
                                : i < step
                                    ...
                                    : 'text-hare/30 cursor-not-allowed'
                                }`}
                        >
                ...
                {selectedSubject && (
                    <span className="text-hare ml-auto hidden sm:inline text-[9px] truncate max-w-[120px] md:max-w-[200px] font-medium">
                    ...
                    <motion.div key="step-exam" variants={fadeUp} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.25 }} className="space-y-4">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-[10px] text-hare font-medium mt-0.5">প্রাক্টিস শুরু করতে একটি এক্সাম বাছাই করো</p>
                            </div>
                            <span className="text-[10px] font-bold text-hare bg-eel px-2.5 py-1 rounded-full">{data.exams.filter(e => e.active).length}টি অ্যাকটিভ</span>
                        </div>
                ...
                {step === 1 && selectedExam && (
                    <motion.div key="step-subject" variants={fadeUp} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.25 }} className="space-y-3">
                        <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="flex items-center justify-between">
                            ...
                            <span className="text-[10px] font-bold text-hare">{selectedExam.subjects?.length || 0}টি সাবজেক্ট</span>
                        </motion.div>
