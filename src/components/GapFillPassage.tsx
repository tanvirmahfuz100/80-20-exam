      ...
      <div className="flex-1 overflow-y-auto min-h-0 space-y-3 px-0.5">
        ...
        {boxWords?.length > 0 && (
          <div className="flex flex-wrap items-center gap-2.5 pt-1 pb-1" aria-label="Available words">
            {boxWords.map((word) => (
              <span key={word} className="px-4 py-2 rounded-xl bg-primary/10 border border-primary/25 text-primary text-[11px] font-black uppercase tracking-widest">
                {word}
              </span>
            ))}
          </div>
      ...
      <div className="shrink-0 sticky bottom-3 px-0 safe-bottom pt-0.5">
        {answeredCount > 0 ? (
          <button
            ...
            className="w-full py-3.5 px-6 bg-[#2F80ED] hover:bg-[#2F80ED]/90 text-white font-black rounded-xl text-sm uppercase tracking-widest transition-all active:scale-[0.97]"
          >
        ...
        {activePopover && activeBlankData && activePopover.isMobile && createPortal(
          <div className="fixed inset-0 z-[100]">
            ...
            <motion.div
              ...
              className="absolute bottom-0 left-0 right-0 bg-surface rounded-t-3xl p-5 safe-bottom max-h-[60vh] flex flex-col"
            >
